...
print(df.head())
pairs = []
for i in range(len(df["img1"])):
    pair = []
    pair.append(df["img1"][i].replace('\\', '/'))
    pair.append(df["img2"][i].replace('\\', '/'))
    pair.append(df["label"][i])
    pairs.append(pair)


pd2 = pd.DataFrame(pairs, columns=['img1', 'img2', 'label'])
pd2 = pd2.to_csv("output1.csv",index=False)